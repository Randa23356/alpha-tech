...
try {
    ...
    $error = "Fitur login dengan Google sedang tidak tersedia.";
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$username || !$password) {
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            notifyLogin($user['id'], $user['username']);
            
            // Save user_id to localStorage for FCM syncing
            echo "<script>
                localStorage.setItem('user_id', '" . $user['id'] . "');
                window.location.href = '" . ($user['role'] === 'admin' ? url('admin') : ($user['role'] === 'korti' ? url('korti') : url('dashboard'))) . "';
            </script>";
            exit();
        } else {
...
<body class="min-h-screen flex items-center justify-center p-4" style="background: linear-gradient(135deg, <?= $primary_color ?> 0%, <?= $secondary_color ?> 100%);">
    <div class="w-full max-w-md">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-blue-200">Silakan login untuk melanjutkan</p>
        </div>

        <!-- Login Card -->
        <div class="bg-white rounded-2xl shadow-2xl p-8">
            <h2 class="text-2xl font-bold mb-6 text-center" style="color: <?= $primary_color ?>;">Login</h2>
            ...
            <div class="mt-6 text-center">
                ...
                <a href="<?= url('register') ?>" class="font-bold hover:underline transition flex items-center justify-center gap-2" style="color: <?= $primary_color ?>;" onmouseover="this.style.color='<?= $secondary_color ?>'" onmouseout="this.style.color='<?= $primary_color ?>'">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Google Login Button -->
<a href="<?= isset($googleLoginUrl) ? htmlspecialchars((string)$googleLoginUrl) : '#' ?>" 
